...
1. **ガバメントAI 源内 OSS の UI の自動生成** … `gwr ui-spec` が出すリクエスト形式 JSON。
...
required = true       # 必須か（省略時 false）。空文字・空白のみは弾かれる
...
- **`required = true` は「キーの存在」ではなく「値が入っていること」を見ます。**
  キー自体が無ければ `REQUIRED_MISSING`、キーはあるが**空**なら `REQUIRED_EMPTY`。
  空の判定は前後の空白を除いたうえで行い、**半角/全角スペース・タブ・改行のみも空**とみなします
  （HTML5 の `required` と意味を揃えるため）。値そのものは trim せずに束縛します。
  `default_value` があれば未指定時にそれが入る。

> ファイルの受信形式（源内OSS 本来の `inputs.files[]` 集約形と per-key 形の両対応）は
...
委譲ノード（`llm` / `retrieval` / `code_interpreter`）の失敗も、この方針の対象です。

- **既定（`fail`）では止まります。** 委譲先に繋がらない・認証に失敗した・時間内に返って
  こないといった失敗は、そのステップでフロー全体を失敗させ、`error.reason` に
  `DELEGATE_*`（下記）を返します。
- **`skip` / `default` を明示したときだけ続行します。**
- **続行させると、結果が欠落したまま成功応答が返ります。** 例えば要約の `llm` ステップに
  `on_error = "default"` を付けると、委譲先が落ちていても `default` の値（空文字など）が
  そのまま `outputs` に載り、利用者からは「要約が空の成功」に見えます。委譲ノードに
  `skip` / `default` を付けるのは、**欠落しても意味が通る**出力に限ってください。
...
Envelope のスロットを、源内OSS のレスポンスの `outputs`（テキスト）／`artifacts`（ファイル）へ
...
`error.reason` に入る値のうち、**利用者向けの個別メッセージを持つもの**です。
これ以外の理由コードは「処理を完了できませんでした。」という汎用メッセージになります
（フロー作成者向けの誤りで、利用者に個別の説明をしても直せないため）。

> この表は実装の `_USER_MESSAGES` と**テストで突合**しています（`tests/test_user_messages.py`）。
> 片方だけ増減するとテストが落ちます。

| `reason` | 出る場面 | 利用者向けメッセージ | 典型的な直し方 |
|---|---|---|---|
| `REQUIRED_MISSING` | `required = true` の入力のキー自体が来ていない。 | 必須の入力が不足しています。 | その項目を送る。フォーム経由なら入力欄が出ているか、`--set <key>=<値>` を付け忘れていないか確認する。 |
| `REQUIRED_EMPTY` | `required = true` の入力が空（空文字・空白のみ）。 | 必須の入力が空です。 | その項目に値を入れる。半角/全角スペース・タブ・改行だけでは通らない。 |
| `NOT_NUMERIC` | `type = "number"` に数値化できない値が来た。 | 数値であるべき項目が数値ではありません。 | 数字だけを入れる（桁区切りや単位を外す）。 |
| `BELOW_MIN` | `number` の値が `min` を下回った。 | 入力値が下限を下回っています。 | `min` 以上の値にする。 |
| `ABOVE_MAX` | `number` の値が `max` を超えた。 | 入力値が上限を超えています。 | `max` 以下の値にする。 |
| `TOO_SHORT` | `text` / `textarea` が `min_length` 未満。 | 入力が短すぎます。 | `min_length` 以上の長さにする。 |
| `TOO_LONG` | `text` / `textarea` が `max_length` 超過。 | 入力が長すぎます。 | `max_length` 以下に縮める。 |
| `ROW_LIMIT_EXCEEDED` | `file.load` の行数が上限（既定 100,000）を超えた。 | ファイルの行数が上限を超えています。 | 行数を減らすか、step の `max_rows` を上げる。 |
| `FILE_PARSE_ERROR` | csv/xlsx として読めなかった。 | ファイルを読み込めませんでした。 | 対応形式（csv / xlsx）で保存し直す。破損していないか確認する。 |
| `FILE_DECODE_ERROR` | base64 として復号できなかった。 | ファイルを読み込めませんでした。 | 送信側のエンコードを確認する（通常は送信経路の不具合）。 |
| `DELEGATE_ENDPOINT_INVALID` | 委譲先（LLM / RAG / Code Interpreter）の接続先が公開 HTTPS として不正。 | 外部サービスの接続先設定に問題があります。管理者に連絡してください。 | 運用者が `GWR_*_ENDPOINT` を見直す（https か、ホスト名があるか、内部宛でないか）。 |
| `DELEGATE_AUTH_FAILED` | 委譲先の認証に失敗した（トークン取得の失敗、または 401 / 403）。 | 外部サービスの認証に失敗しました。管理者に連絡してください。 | 運用者が `GWR_*_API_KEY` や Keycloak の資格情報を見直す。 |
| `DELEGATE_TIMEOUT` | 委譲先が時間内に応答しなかった（タイムアウト、ポーリング打ち切り、429）。 | 外部サービスが混み合っているか、応答が時間内に返りませんでした。時間をおいて再実行してください。 | 時間をおいて再実行する。続くなら運用者がタイムアウト値と委譲先の負荷を見直す。 |
| `DELEGATE_FAILED` | 上記以外の委譲失敗（接続失敗、委譲先が返したエラー、想定外の応答）。 | 外部サービスの呼び出しに失敗しました。時間をおいて再実行し、続く場合は管理者に連絡してください。 | 時間をおいて再実行する。続くなら運用者がサーバのログ（トレースバックに url・status・応答本文が出る）を見る。 |

座標（`coordinates`）が付くのは `DataError` 由来のものだけで、入力契約の違反では `null` です。

上表以外の理由コード（`UNKNOWN_NODE_TYPE` / `BRANCH_EVAL_FAILED` / `FOREACH_NOT_LIST` /
`FOREACH_LIMIT_EXCEEDED` / `STEP_LIMIT_EXCEEDED` / 各ノードのエラーなど）は、
**フロー定義側の誤り**なので利用者には汎用メッセージを返します。`error.reason` には
**理由コードだけ**が入り（詳細は応答に載せません）、フロー作成者は `gwr run` の例外
メッセージで詳細を確認します。
...
| 面 | 見る人 | 出るもの |
|---|---|---|
| `error.reason` と利用者向けメッセージ | 利用者 | 理由コードと定型文だけ。**設定値（url・ホスト名・応答本文）は出しません** |
| 起動時のエラーメッセージ・トレースバック・サーバのログ | 運用者・フロー作成者 | url・ホスト名・status・応答本文。**出さないと直せない**ので出します |

委譲先の接続先が不正な場合、`llm` は**起動時**に（接続先はアダプタ生成時に検証されるため）、
`retrieval` / `code_interpreter` は**実行時**に落ちます。前者は `gwr serve` / `gwr run` が
1 行のメッセージを出して終了し、後者は `DELEGATE_ENDPOINT_INVALID` として利用者へ返ります。
