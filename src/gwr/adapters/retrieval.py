"""retrieval ノード ＋ 源内OSS の RAG API アダプタ契約。

source（検索対象）と query を載せて委譲し、正規化済み Doc[] を out に書く。
実体（検索・索引）は源内OSS 側。本モジュールは整形と件数検証のみ。
"""
...
class RetrievalNode(Node):
    ...
    def run(
        ...
        if adapter is None:
            raise NodeError("ADAPTER_MISSING", "adapter=retrieval")
        query = inputs.get("query")
        if not isinstance(query, str):
            raise NodeError("QUERY_MISSING", "input=query expected=str")
